function scores=main_norman_bioconvolving(biometricData,scenario)
...
elseif strcmp(scenario,'Homo_UK') || strcmp(scenario,'Hete_UK')
    for i=1:numel(ID_list),
        ...
        index_template_valid = selected_user{VALID}{i}
        ...
        if strcmp(scenario,'Homo_UK')
            ...
            transformed_data(index_template_valid,:)=bioconvolving(data(index_template_valid,:),key);
        else
        ...
        for iUser=1:numel(userlist)
          ...
          %% Encode the impostor user, encode its data with a key
            transformed_data(index_template_neg,:)=bioconvolving(data(index_template_neg,:),key);  
            transformed_data(index_template_neg_valid,:)=bioconvolving(data(index_template_neg_valid,:),key);
        end
    end;
end

scores=runExperiments(transformed_data,selected_user,ID_list,TRAIN,TRAIN_IMP,VALID,VALID_IMP,TEST,TEST_IMP);
return
end